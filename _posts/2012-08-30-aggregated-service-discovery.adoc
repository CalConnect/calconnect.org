...
CalConnect s link:{{'/tc-autodiscovery' | relative_url}}[AUTODISCOVERY] Technical Committee has submitted the initial draft of a specification on https://datatracker.ietf.org/doc/draft-daboo-aggregated-service-discovery/' | relative_url}}[Aggregated Service Discovery] for clients and mobile devices to the IETF.
